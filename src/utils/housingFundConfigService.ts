...
export class HousingFundConfigService {
  ...
  static async getConfigList(
    ...
    const url = `${API_ENDPOINTS.HOUSING_FUND_CONFIG.LIST}?${queryParams.toString()}`;
    const response = await httpClient.get<HousingFundConfigListResponse>(url);
    return response;
  }

  // 创建公积金配置
  static async createConfig(data: CreateHousingFundConfigRequest): Promise<void> {
    await httpClient.post(API_ENDPOINTS.HOUSING_FUND_CONFIG.CREATE, data);
  }

  // 更新公积金配置
  static async updateConfig(id: string, data: CreateHousingFundConfigRequest): Promise<void> {
    await httpClient.put(`${API_ENDPOINTS.HOUSING_FUND_CONFIG.UPDATE}/${id}`, data);
  }

  // 删除公积金配置
  static async deleteConfig(id: string): Promise<void> {
    await httpClient.delete(`${API_ENDPOINTS.HOUSING_FUND_CONFIG.DELETE}/${id}`);
  }
